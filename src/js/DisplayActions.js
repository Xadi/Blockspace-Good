...
class DisplayActions extends React.Component {
  ...
  handleSignLease(event) {
    ...
    this.smartLeaseInstance.signLease( this.props.index, { from: this.props.account } )
  }

  handlePayDeposit(event) {
    event.preventDefault()
    this.smartLeaseInstance.payDeposit( this.props.index, {from: this.props.account, value: this.props.lease.depositToPay} )
  }

  handlePayRent(event) {
    event.preventDefault()
    this.smartLeaseInstance.payRent( this.props.index, {from: this.props.account, value: this.props.lease.rentPerMonth} )
  }

  handleWithdrawRent(event) {
    event.preventDefault()
    this.smartLeaseInstance.withdrawRent(this.props.index, {from: this.props.account, gas:50000} )
  }

  handleWithdrawDeposit(event) {
    event.preventDefault()
    this.smartLeaseInstance.withdrawDeposit( this.props.index, {from: this.props.account, gas:50000} )
  }
