...
class DisplayTenantContracts extends React.Component {
  ...
  loadLeaseByIndex(_index) {
      ...
      this.smartLeaseInstance.leasesDetails(_index).then((leaseDetails) => {
        ...
        var leasesTenant = [...this.state.leases]
        if (lease[0] === this.props.account ) {
          ...
          this.setState({ leases: leasesTenant })
        }
      })       
...
export default DisplayTenantContracts
