...
class DisplayLitigatorContracts extends React.Component {
  ...
  loadLeaseByIndex(_index) {
      ...
      this.smartLeaseInstance.leasesDetails(_index).then((leaseDetails) => {
        ...
        var leasesLitigator = [...this.state.leases]
        if (lease[0] === this.props.account ) {

          const leasesLitigator = [...this.state.leases]
          leasesLitigator.push({
              landlord: lease[0],
              tenant: lease[1],
              litigator: lease[2],
          ...
          this.setState({ leases: leasesLitigator })
        }
      })       
...
export default DisplayLitigatorContracts
